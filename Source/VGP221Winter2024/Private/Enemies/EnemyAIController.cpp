...
{
	GetWorld()->GetTimerManager().ClearTimer(RetriggerablTimerHandle);
	FunctionDelegate.BindUFunction(this, FName("SetCanSeePlayer"), false, GetPawn());
	GetWorld()->GetTimerManager().SetTimer(RetriggerablTimerHandle, FunctionDelegate, PawnSensing->SensingInterval * 2.0f, false);
}
